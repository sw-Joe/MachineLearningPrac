...
@dataclass
class ClsTracker(BaseMetricTracker):
    """
    accuracy, top-1, top-5 등 요약 지표용 Tracker
    """
    topk: tuple = (1, 5)
    ...
    def synchronize(self):
        super()._sync_common() # Loss, Total 합산
...
#         # Top-k 계산 핵심 로직
#         max_k = max(self.topk)
#     # correct_counts: dict = field(default_factor
#         _, pred = predicts.topk(max_k, 1, True, True)
#         correct_matrix = pred.t().eq(targets.view(1, -1).expand_as(pred.t()))
...
class ClsReportTracker:
    """
    F1-score, 상세 리포트용 Tracker
    """
