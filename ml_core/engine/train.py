...
from metric.classification_metric import ClsTracker, ClsReportTracker
from metric.regression_metric import RegTracker
...
class Trainer:
    def __init__(self, device, model, optimizer, criterion, scheduler=None, ema=None, config=None):
        ...
        # 트래커 초기화
        self.train_tracker = ClsTracker(topk=(1, 5), device=device)
        self.val_tracker = ClsTracker(topk=(1, 5), device=device)
        self.clf_metric = ClsReportTracker(classes=config.dataset.label)
