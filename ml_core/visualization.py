...
class Visualizer(BaseEvaluator):
    ...
    def plot_confidence_grid(self, groups, save_dir):
        ...
        plt.close()
        print("✅ img:'quartile confidence grid' successfully created.")   # feedback
    ...
    def confusion_matrix_visualization(self, y_true, y_pred, save_dir) -> None:
        ...
        if num_classes > MAX_READABLE_CLASSES:
            print(f"⚠️ 현재 클래스 수({num_classes}개)가 너무 많아 시각화 결과의 가독성이 떨어질 수 있습니다.")
            print(f"추천 최대 클래스 수는 {MAX_READABLE_CLASSES}개 이하입니다.")
        ...
        plt.close()
        print("✅ img:'confusion matrix' successfully created.")   # feedback



def regression_scatterplot_visualization(y_true, y_pred, save_dir, timestamp) -> None:
    """회귀 분석용 산점도 시각화"""
    ...
    # 산점도 및 y=x 라인
    plt.scatter(y_true, y_pred, alpha=0.5, s=20, c='royalblue', label='Predictions')
    min_val, max_val = min(y_true.min(), y_pred.min()), max(y_true.max(), y_pred.max())
    plt.plot([min_val, max_val], [min_val, max_val], color='darkorange', linestyle='--', label='Ideal ($y=x$)')

    stats_text = f"$R^2 = {r2:.4f}$\n$MAE = {mae:.4f}$"
    plt.text(0.05, 0.95, stats_text, transform=plt.gca().transAxes, verticalalignment='top', 
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

    plt.xlabel("Ground Truth", fontsize=12, fontweight='bold')
    plt.ylabel("Prediction", fontsize=12, fontweight='bold')
    plt.title(f"Regression Prediction Analysis\nTime: {timestamp}", fontsize=15, pad=20)
    plt.savefig(f"{save_dir}/regression_scatterplot_{timestamp}.png", dpi=150)
    plt.close()
    print("✅ img:'scatterplot' successfully created.")   # feedback
